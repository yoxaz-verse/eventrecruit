...
export default async function OrganizerDashboard({searchParams}:{searchParams:Promise<{filter?:string;message?:string}>}) {
 ...
 <div className="grid gap-4 lg:grid-cols-2">{events.filter(e=>active==='all'||eventPhase(e)===active).map(e=><article className="panel p-6" key={e.id}><span className="badge capitalize">{eventPhase(e)}</span><h2 className="my-3 text-2xl font-bold">{e.title}</h2><p>{e.city||'Location to be added'} · {e.starts_at||'Dates to be added'}{e.ends_at?` – ${e.ends_at}`:''}</p><div className="mt-4 flex gap-2"><Link className="button button-secondary" href={`/dashboard/organizer/events/${e.id}`}>Manage event</Link><Link className="button button-secondary" href={`/dashboard/organizer/events/${e.id}/participants`}>Exhibitors</Link></div></article>)}</div>
