...
export default async function AgencyReputation({searchParams}:{searchParams:Promise<{client?:string}>}) {
  const [clients,params,{db}]=await Promise.all([agencyClients(),searchParams,requireAgencyWorkspace()]), selected=selectedAgencyClient(clients,params.client);
  ...
  const {data:events}=selected?await db.from("events").select("staffing_roles(id,title,placements(id,talent_id,staffing_role_id,status,placement_reviews(placement_id,reviewer_exhibitor_id)))").eq("exhibitor_id",selected.exhibitorId):{data:[]};
  const roles=(events??[]).flatMap(event=>event.staffing_roles??[]);
  const placements=roles.flatMap(role=>(role.placements??[]).filter(placement=>placement.status==="completed"));
  const pending=placements.filter(placement=>!(placement.placement_reviews??[]).some(review=>review.reviewer_exhibitor_id===selected?.exhibitorId));
