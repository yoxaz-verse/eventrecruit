  ...
  return <DashboardShell active="exhibitor" current="/dashboard/exhibitor/applicants">
    ...
    <div className="grid gap-4">{applications.map(application => {
      ...
      return <article className="panel p-5" key={application.id}>
        <div className="flex items-start gap-4">{person?.avatar_url ? <img className="h-16 w-16 rounded-full border border-[var(--line)] object-cover" src={`/api/media/talent/${application.talent_id}`} alt={`${person.full_name} profile picture`}/> : null}<div><span className="badge capitalize">{application.status}</span><h2 className="mt-3 text-xl font-bold">{person?.full_name ?? "Applicant"}</h2><p>{application.role} · {application.event} · {application.workDays}</p><p className="text-sm">Confirmed roster: {application.booked}/{application.headcount}</p>{application.cancellation_requested_at ? <p className="text-sm font-bold">Cancellation requested — choose Cancelled to release this booking.</p> : null}<p className="text-sm text-[var(--muted)]">{person?.city ?? "City not provided"}</p></div></div>
        <WorkflowActionForm action={updateApplicationStatus} className="mt-4 flex flex-wrap items-center gap-3"><input type="hidden" name="application_id" value={application.id}/><label className="label">Update status<select className="input" name="status" defaultValue={application.status}>{["applied", "shortlisted", "accepted", "rejected", "completed", "cancelled"].map(status => <option key={status} value={status}>{status}</option>)}</select></label><SubmitButton pendingText="Updating…">Save</SubmitButton></WorkflowActionForm>
      </article>;
