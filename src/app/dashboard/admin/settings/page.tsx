...
export default async function AdminSettings() {
  ...
  const [pendingResult, disabledResult] = db ? await Promise.all([
    db.from("account_deletion_requests").select("id,profile_id,requested_at,profiles(full_name,role)").eq("status", "pending").order("requested_at"),
  ...
  return <DashboardShell active="admin" current="/dashboard/admin/settings">
    <div className="mx-auto max-w-4xl">
      ...
      <AccountSettingsPanel emailNotifications={settings.emailNotifications} inAppNotifications={settings.inAppNotifications} deletionPending={settings.deletionPending}/>
