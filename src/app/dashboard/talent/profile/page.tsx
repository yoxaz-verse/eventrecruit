...
export default async function TalentProfilePage({searchParams}:{searchParams:Promise<{saved?:string}>}) {
  ...
  return <DashboardShell active="talent" current="/dashboard/talent/profile"><div className="max-w-3xl"><span className="badge badge-accent">Talent profile</span><h1 className="mt-3 text-4xl font-black">Your profile and locations</h1><p className="mt-2 text-[var(--muted)]">Your selected work cities control matching alerts. Professional details can be completed whenever you are ready.</p>{saved?<p className="alert mt-5" role="status">Profile saved.</p>:null}<AuthActionForm action={saveTalentProfile} className="panel mt-6 grid gap-5 p-6">
