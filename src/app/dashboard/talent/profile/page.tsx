...
export default async function TalentProfilePage({searchParams}:{searchParams:Promise<{saved?:string;returnTo?:string}>}) {
  ...
  return <DashboardShell active="talent" current="/dashboard/talent/profile"><div className="max-w-3xl"><span className="badge badge-accent">Talent profile</span><h1 className="mt-3 text-4xl font-black">Your profile and locations</h1><p className="mt-2 text-[var(--muted)]">Use a clear, recent profile photo. Your selected work cities control matching alerts, and this reusable profile is used for every application.</p>{saved?<p className="alert mt-5" role="status">Profile and preferences saved.</p>:null}<div className="panel mt-6 p-6"><ProfileImageForm kind="Profile picture" imageUrl={profile.data.avatar_url ? `/api/media/talent/${talent.id}` : null}/></div><AuthActionForm action={saveTalentProfile} className="panel mt-6 grid gap-5 p-6">
    ...
    <SubmitButton pendingText="Saving profile…">Save profile and preferences</SubmitButton>
