...
export default async function ApplicationReviewPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{current?:string}>}){
 ...
 return <><div className="max-w-2xl"><span className="badge badge-accent">Application check</span><h1 className="mt-3 text-4xl font-black">Review before applying</h1><p className="mt-2 text-[var(--muted)]">{role.data.title} · {event?.title}</p>
