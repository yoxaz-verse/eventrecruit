...
export async function requestBookingCancellation(form:FormData) {
  ...
  const {data,error} = await db.from("applications").update({cancellation_requested_at:new Date().toISOString()}).eq("id",id).eq("talent_id",talent.id).eq("status","accepted").select("id").maybeSingle();
