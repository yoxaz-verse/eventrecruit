...
export async function submitSpaceInquiry(_state:InquiryState,data:FormData):Promise<InquiryState> {
 ...
 if(profile?.role!=='exhibitor') return {error:'Sign in with an exhibitor account to inquire.',success:''};
 ...
 const {error}=await db.from('event_space_inquiries').insert({event_id:eventId,offer_id:offerId,exhibitor_id:profile.id,requested_area_sqft:area,requested_units:units,message});
