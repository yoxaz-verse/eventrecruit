...
export default async function EventsPage({searchParams}:{searchParams:Promise<{q?:string;city?:string}>}) {
 ...
 const result=db?await db.from('organizer_events').select('id,title,description,city,venue,starts_at,ends_at,company_id,status,published_at,staffing_needs').eq('status','published').gte('ends_at',indiaToday()).order('starts_at'):null;
 ...
 return <div className="shell public-page"><TopNav/><main className="page py-12"><div className="public-page-intro"><span className="badge">Discover events</span><h1>Upcoming events</h1><p>Explore what event companies are organizing across India.</p></div>
 ...
 {unavailable?<p role="alert" className="panel p-6">Events are temporarily unavailable. Please try again later.</p>:<div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filtered.map(e=><article className="panel event-card gap-3 p-6" key={e.id}><span className="badge self-start">{e.city}</span><h2 className="text-2xl font-bold"><Link href={`/events/${e.id}`}>{e.title}</Link></h2><p>{companyName(e.company_id)}</p><p className="text-sm">{e.starts_at} – {e.ends_at}</p><p className="line-clamp-3 text-[var(--muted)]">{e.description}</p><EventStaffingNeeds needs={e.staffing_needs}/><Link className="button button-secondary" href={`/events/${e.id}`}>View event</Link></article>)}</div>}
