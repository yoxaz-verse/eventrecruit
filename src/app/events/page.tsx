...
export default async function EventsPage({searchParams}:{searchParams:Promise<{q?:string;city?:string}>}) {
 ...
 const [result,companiesResult,submittedResult]=db?await Promise.all([
  db.from('organizer_events').select('id,title,description,city,venue,starts_at,ends_at,company_id,status,published_at,staffing_needs,booking_url,event_type,venue_setting,requirement_sections,requirement_details').eq('status','published').gte('ends_at',indiaToday()).order('starts_at'),
  db.from('organizer_companies').select('id,name,city,description,website'),
  db.from('exhibitor_event_submissions').select('id,title,description,city,venue,starts_at,ends_at').eq('status','approved').gte('ends_at',indiaToday()).order('starts_at'),
 ]):[null,null,null];
