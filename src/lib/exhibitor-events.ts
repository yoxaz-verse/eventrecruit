...
export async function selectableEvents(exhibitorId?: string): Promise<SelectableEvent[]> {
  ...
  const [organizers, submissions] = await Promise.all([
    ...
    exhibitorId
      ? db.from("exhibitor_event_submissions").select("id,title,venue,city,starts_at,ends_at,status").eq("exhibitor_id",exhibitorId).in("status",["pending","approved"]).gte("ends_at", today)
      : db.from("exhibitor_event_submissions").select("id,title,venue,city,starts_at,ends_at,status").eq("status", "approved").gte("ends_at", today),
  ]);
