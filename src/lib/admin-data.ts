...
type Params={q:string;status:string;sort:"newest"|"oldest";page:number;view:string};
export type AdminDataset={key:string;title:string;entity?:string;statusKey?:string;rows:Record<string,unknown>[];total:number};
const configs:Record<AdminSection,Array<{key:string;title:string;table:string;select:string;search?:string;status?:string;entity?:string}>>={
 ...
 workforce:[{key:"roles",title:"Staffing roles",table:"staffing_roles",select:"id,title,headcount,hourly_rate,work_starts_on,work_ends_on,status,created_at",search:"title",status:"status",entity:"staffing_role"},{key:"applications",title:"Applications",table:"applications",select:"id,talent_id,staffing_role_id,status,cancellation_requested_at,created_at,talent_profiles(profiles(full_name)),staffing_roles(title,events(title))",status:"status",entity:"application"},{key:"placements",title:"Placements",table:"placements",select:"id,talent_id,staffing_role_id,agreed_rate,status,completed_at,created_at,talent_profiles(profiles(full_name)),staffing_roles(title,events(title))",status:"status",entity:"placement"}],
...
export function adminSectionViews(section:AdminSection){return configs[section].map(({key,title})=>({key,title}));}

export async function loadAdminSection(section:AdminSection,params:Params):Promise<{dataset:AdminDataset;hasNext:boolean}>{
 ...
 const config=configs[section].find(item=>item.key===params.view)??configs[section][0];
   let query=db.from(config.table).select(config.select,{count:"exact"}).order(config.select.includes("created_at")?"created_at":"updated_at",{ascending:params.sort==="oldest"}).range(from,to);
   ...
   const {data,error,count}=await query;if(error&&error.code!=="PGRST205")throw new Error(`Unable to load ${config.title.toLowerCase()}.`);
   const rows=(data??[]) as unknown as Record<string,unknown>[];
   if(config.key==="applications"||config.key==="placements"){
    for(const row of rows){
     const talent=one(row.talent_profiles) as Record<string,unknown>|null;
     const profile=one(talent?.profiles) as Record<string,unknown>|null;
     const role=one(row.staffing_roles) as Record<string,unknown>|null;
     const event=one(role?.events) as Record<string,unknown>|null;
     row.talent_name=profile?.full_name??null;
     row.role_title=role?.title??null;
     row.event_title=event?.title??null;
    }
   }
   for(const row of rows)if(!row.id)row.id=row.profile_id??row.exhibitor_id;
   const dataset={key:config.key,title:config.title,entity:config.entity,statusKey:config.status,rows:rows.slice(0,ADMIN_PAGE_SIZE),total:count??0};
 return {dataset,hasNext:rows.length>ADMIN_PAGE_SIZE};
}
...
export async function adminCounts(){
 ...
 const values=await Promise.all(items.map(async([label,table])=>{const {count,error}=await db.from(table).select("id",{count:"exact",head:true});if(error&&error.code!=="PGRST205")throw new Error(`Unable to count ${label}.`);return {label,value:count??0};}));
 return values;
}
