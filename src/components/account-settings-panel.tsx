...
export function AccountSettingsPanel({ emailNotifications, inAppNotifications, deletionPending }: { emailNotifications: boolean; inAppNotifications: boolean; deletionPending: boolean }) {
  return <div className="grid gap-6">
    <form action={saveAccountPreferences} className="panel grid gap-4 p-5 sm:p-6">
      ...
      <label className="flex items-start gap-3 rounded-xl border border-[var(--line)] p-4"><input className="mt-1" type="checkbox" name="email_notifications" defaultChecked={emailNotifications} /><span><strong className="block">Email notifications</strong><small className="text-[var(--muted)]">Operational updates and account activity. Security messages are always sent.</small></span></label>
      <label className="flex items-start gap-3 rounded-xl border border-[var(--line)] p-4"><input className="mt-1" type="checkbox" name="in_app_notifications" defaultChecked={inAppNotifications} /><span><strong className="block">In-app notifications</strong><small className="text-[var(--muted)]">Show relevant updates inside your dashboard.</small></span></label>
      <button className="button button-primary w-full sm:w-fit">Save preferences</button>
    </form>
