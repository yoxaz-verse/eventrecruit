...
export function ExternalClientForm() {
  ...
  return <form action={action} className="panel grid gap-4 p-5"><h2 className="text-xl font-black">Create an external client</h2><p className="text-sm text-[var(--muted)]">Create a reusable exhibitor identity now and optionally reserve it for a future account email.</p><label className="label">Company name<input className="input" name="company_name" required maxLength={160}/></label><label className="label">Industry<input className="input" name="industry" maxLength={160}/></label><label className="label">Claim email (optional)<input className="input" name="claim_email" type="email" maxLength={254}/></label>{state.error?<p className="alert" role="alert">{state.error}</p>:null}{state.success?<p className="alert" role="status">{state.success}</p>:null}<SubmitButton pendingText="Creating…">Create managed client</SubmitButton></form>;
}
