...
export function EventForm({event,slots=[],offers=[],locations=[],companies=[]}:{event?:OrganizerEvent;slots?:BookingSlot[];offers?:SpaceOffer[];locations?:LocationOption[];companies?:Company[]}) {
 ...
 <label className="label">Venue name<input className="input" name="venue" required maxLength={200} defaultValue={event?.venue??''} placeholder="For example: Lulu International Convention Centre"/></label>
 <VenueLocationPicker locations={locations} initialLocationId={event?.location_id} initial={event ? {city:event.city,label:event.location_label??undefined,latitude:event.latitude??undefined,longitude:event.longitude??undefined,countryCode:event.location_country_code==='IN'?'IN':undefined} : undefined}/>
